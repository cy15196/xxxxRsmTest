...
SerialPortTransfer::SerialPortTransfer(TopTest::DeviceType deviceType,QObject *parent)
    : QObject(parent),deviceType(deviceType)
{
...
{
    if(RSMUsb::instance()->getPortState() != RSMUsb::RefreshPort)
        return;

    qint64 currentTimeid = this->getTimeId();
...
//                continue;
    ...
    if(!portNameList.isEmpty() && currentTimeid == RSMUsb::instance()->getTimeId())
    {
...
//    serial->reset();
    ...
    emit this->portOpenSuccess(activePortName);
...
{
    if(RSMUsb::instance()->getPortState() != RSMUsb::Working)
        return;
...
qint64 SerialPortTransfer::getTimeId() const
{
    return timeId;
}

void SerialPortTransfer::setTimeId(const qint64 &value)
{
    timeId = value;
}
