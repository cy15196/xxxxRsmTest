...
QStringList ChartAxisElement::createStepValueLabels(qreal min, qreal max, double step,
                                                const QString &format) const
{
    QStringList labels;
    ...
    if (format.isNull()) {
        int n = qMax(int(-qFloor(std::log10(step))), 0) + 1;
        qreal nowValue =  min;
        while(nowValue < max)
        {
            labels << presenter()->numberToString(nowValue, 'f', n);
            nowValue += step;
        }
    } else {
        QByteArray array = format.toLatin1();
        ...
        while(nowValue < max)
        {
            labels << formatLabel(formatSpec, array, nowValue, precision, preStr, postStr);;
            nowValue += step;
        }
    }
