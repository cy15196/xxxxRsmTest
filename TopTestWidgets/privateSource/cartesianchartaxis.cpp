...
void CartesianChartAxis::updateMinorTickItems()
{
    ...
    } else if (axis()->type() == ClabsoStepAxisType) {
        ClabsoStepAxis *stepAxis = qobject_cast<ClabsoStepAxis *>(axis());

        expectedCount = qFloor(( max() - min() ) / stepAxis->minorTickStep() )
                - stepAxis->tickCount() + 1;
        expectedCount = qMax(expectedCount, 0);
        qDebug()<<"exprectedCount"<<expectedCount;
