...
void HorizontalAxis::updateMinorTickGeometry()
{
    ...
    if(axis()->type() == ClabsoStepAxisType) {
           const ClabsoStepAxis *stepAxis = qobject_cast<ClabsoStepAxis *>(axis());

           tickSpacing = stepAxis->tickStep();

           qreal tickSpace = stepAxis->minorTickStep();
           minorTickCount = 0;

           while(tickSpace < stepAxis->tickStep())
           {
               minorTickSpacings.append(tickSpace);
               tickSpace += stepAxis->minorTickStep();
               minorTickCount++;
           }
    ...
    else if (minorTickCount < 1 || tickSpacing == 0.0 || minorTickSpacings.count() != minorTickCount)
        return;
