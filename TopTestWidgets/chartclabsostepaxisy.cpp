...
ChartClabsoStepAxisY::ChartClabsoStepAxisY(ClabsoStepAxis *axis, QGraphicsItem *item)
...
ChartClabsoStepAxisY::~ChartClabsoStepAxisY()
{
}

QVector<qreal> ChartClabsoStepAxisY::calculateLayout() const
{
    int tickCount = m_axis->tickStep();
...
void ChartClabsoStepAxisY::updateGeometry()
{
...
void ChartClabsoStepAxisY::handleTickCountChanged(int tick)
{
    Q_UNUSED(tick);
...
void ChartClabsoStepAxisY::handleMinorTickCountChanged(int tick)
{
    Q_UNUSED(tick);
    QGraphicsLayoutItem::updateGeometry();
    if (presenter())
...
void ChartClabsoStepAxisY::handleLabelFormatChanged(const QString &format)
{
    Q_UNUSED(format);
    QGraphicsLayoutItem::updateGeometry();
    if(presenter()) presenter()->layout()->invalidate();
...
QSizeF ChartClabsoStepAxisY::sizeHint(Qt::SizeHint which, const QSizeF &constraint) const
{
    Q_UNUSED(constraint)

    QSizeF sh;
    QSizeF base = VerticalAxis::sizeHint(which, constraint);
    QStringList ticksList = createValueLabels(min(),max(),m_axis->tickStep(),m_axis->labelFormat());
    qreal width = 0;
...
#include "moc_chartclabsostepaxisy_p.cpp"
