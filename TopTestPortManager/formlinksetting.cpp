...
//    connect(TopTestPortManager::instance(),SIGNAL(aviablePortListChanged(QStringList)),
...
void FormLinkSetting::on_comboBox_linkType_currentIndexChanged(const QString &arg1)
{
    ...
    TopTestPortManager::instance()->setBauderate(ui->comboBox_linkType->currentData().toInt());
...
{
    qDebug() <<"FormLinkSetting::onAviablePortChanged"<<portList;
    ...
    else if(!portList.contains(TopTestPortManager::instance()->activePort()))
        ui->comboBox_deviceChoose->setCurrentText(portList.first());
    else
        ui->comboBox_deviceChoose->setCurrentText(TopTestPortManager::instance()->activePort());
...
{
    TopTestPortManager::instance()->blockSignals(true);
    disconnect(TopTestPortManager::instance(),SIGNAL(aviablePortListChanged(QStringList)),
            this,SLOT(onAviablePortChanged(QStringList)));

    qDebug()<<"FormLinkSetting::resetLink"<<true;
    TopTestPortManager::instance()->closeCurrentPort();
//    RSMUsb::instance()->setActivePortName(QString());
    ...
    TopTestPortManager::instance()->blockSignals(false);
}
