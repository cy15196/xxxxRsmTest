...
QUICreator::QUICreator(QWidget *parent) : QMainWindow(parent), ui(new Ui::QUICreator)
{
    ui->setupUi(this);
    this->initForm();
}
...
void QUICreator::initForm()
{
    ...
    on_btnNew_clicked();
    QTimer::singleShot(1000, this, SLOT(initPanelWidget()));

    //设置tab选项卡的最小宽度高度
    QString qss = "QTabWidget#tabWidget>QTabBar::tab{min-width:50px;min-height:20px;}";
...
void QUICreator::initPanelWidget()
{
    ...
    //加载内置图标
    QGridLayout *layout = (QGridLayout *)ui->widgetIcon->layout();
    int index = 0;
    for (int row = 0; row < 7; row++) {
        for (int col = 0; col < 10; col++) {
            enum QStyle::StandardPixmap icon = (enum QStyle::StandardPixmap)index;
            QPushButton *btn = new QPushButton(QApplication::style()->standardIcon(icon), QString::number(index + 1));
...
{
    QUIHelper::showMessageBoxInfo("恭喜你,中了500万大奖!请到飞扬青云处领取大奖!");
    //QUIHelper::showMessageBoxInfo("恭喜你,中了500万大奖!请到飞扬青云处领取大奖!", 3, true);
}

void QUICreator::on_btnQuestion_clicked()
{
    if (QUIHelper::showMessageBoxQuestion("确定不要当前的500万大奖吗?") == QMessageBox::Yes) {
        QUIHelper::showMessageBoxInfo("恭喜你,中了500万大奖!请到飞扬青云处领取大奖!");
    } else {
        QUIHelper::showMessageBoxError("领取500万大奖发生错误,请重试!");
    }
}
...
{
    QUIHelper::showMessageBoxError("领取500万大奖发生错误,请重试!");
    //QUIHelper::showMessageBoxError("领取500万大奖发生错误,请重试!", 0, true);
}

void QUICreator::on_btnInput_clicked()
{
    QString result = QUIHelper::showInputBox("请输入姓名:");
    if (!result.isEmpty()) {
        QUIHelper::showMessageBoxInfo(QString("您输入的姓名是 : %1").arg(result));
    }
}

void QUICreator::on_btnInputPwd_clicked()
{
    QString result = QUIHelper::showInputBox("请输入领取500万大奖密码:", 0, 0, "领取500万大奖密码", true);
    if (!result.isEmpty()) {
        QUIHelper::showMessageBoxInfo(QString("您输入的密码是 : %1").arg(result));
    }
}
...
void QUICreator::on_btnInputcbox_clicked()
{
    ...
    QString result = QUIHelper::showInputBox("请选择学号:", 1, 0, defaultValue.join("|"));
    if (!result.isEmpty()) {
        QUIHelper::showMessageBoxInfo(QString("您选择的学号是 : %1").arg(result));
    }
}

void QUICreator::on_btnSelectDate_clicked()
{
    QString dateStart, dateEnd;
    QUIHelper::showDateSelect(dateStart, dateEnd);
    if (!dateStart.isEmpty() && !dateEnd.isEmpty()) {
        QUIHelper::showMessageBoxInfo(QString("开始时间: %1\n结束时间: %2").arg(dateStart).arg(dateEnd));
    }
}
...
{
    QColor color = QColorDialog::getColor(QColor(txt->text()), this, "选择颜色", QColorDialog::ShowAlphaChannel);
    if (!color.isValid()) {
        return;
    }
...
void QUICreator::on_btnAbout_clicked()
{
    qApp->aboutQt();
}
...
{
    QString fileName = ":/qss/lightblue.css";
...
void QUICreator::on_btnOpen_clicked()
{
    ...
    if (!fileName.isEmpty()) {
        ...
        if (file.open(QFile::ReadOnly)) {
            ...
            //临时替换样式表中的 :/qss/ 为绝对文件路径
            QFileInfo fileInfo(fileName);
            str = str.replace(":/qss", fileInfo.path());
